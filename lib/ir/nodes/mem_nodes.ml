...
let create_new g loc ?parent_fun ~ctrl ~mem ~size typ =
    ...
    Node.G.add_node g n { Node.mem = Some (AnyMem mem); size = Some (AnyData size) };
...
let create_addr_of g loc ?parent_fun place =
    let n = Node.create_data ?parent_fun loc (Ptr place.Node.typ) AddrOf in
    Node.G.add_node g n { Node.place = Some (AnyData place); offset = None };
    n

let create_addr_of_field g loc ?parent_fun place ?index field_name =
    let t = Types.get_field_type place.Node.typ field_name |> Option.value ~default:ALL in
    ...
    let n = Node.create_data ?parent_fun loc (Ptr t) (AddrOfField field_name) in
    Node.G.add_node g n
      {
        Node.place = Some (AnyData place);
        offset = Option.map index ~f:(fun index -> Node.AnyData index);
      };
    n
...
let create_deref g loc ?parent_fun ~mem ptr =
    let t =
        ...
        | _ -> assert false
    in
    let n = Node.create_data ?parent_fun loc t Deref in
    Node.G.add_node g n { Node.mem = Some (AnyMem mem); ptr = Some (AnyData ptr) };
    n
...
let create_copy ?parent_fun g loc ~mem ~src ~dst =
    ...
    Node.G.add_node g n
      { Node.mem = Some (AnyMem mem); src = Some (AnyData src); dst = Some (AnyData dst) };
    n
